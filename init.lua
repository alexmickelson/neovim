  ...
  local servers = {
    ...
    gopls = {
      root_dir = function(bufnr, on_dir)
        ...
        if fname:match '^diffview://' then return end
...
do
  ...
  local function is_editor_window(winid)
    if not vim.api.nvim_win_is_valid(winid) or vim.api.nvim_win_get_config(winid).relative ~= '' then return false end
  ...
  vim.api.nvim_create_autocmd('WinEnter', {
    ...
    callback = function()
      ...
      if is_editor_window(winid) then last_editor_window = winid end
    end,
  })

  local function most_recent_editor_window()
    if last_editor_window and is_editor_window(last_editor_window) then return last_editor_window end
    for _, winid in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
      if is_editor_window(winid) then return winid end
    end
  end
...
do
  ...
  vim.api.nvim_create_autocmd('BufEnter', {
    pattern = '*',
    callback = function(event)
      if vim.bo[event.buf].buftype == 'terminal' then vim.schedule(function() vim.cmd.startinsert() end) end
    end,
  })
...
do
  ...
  vim.api.nvim_create_autocmd({ 'InsertLeave', 'BufLeave' }, {
    ...
    callback = function(event)
      if not vim.api.nvim_buf_is_valid(event.buf) or not vim.api.nvim_buf_is_loaded(event.buf) then return end
